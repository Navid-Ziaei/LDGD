...
class VariationalStrategyJointGPLVM(_VariationalStrategy):
    r"""
    ...
    def __init__(
            self,
            model: ApproximateGP,
            inducing_points_count: Tensor,
            inducing_points_cat: Tensor,
            variational_distribution: _VariationalDistribution,
            learn_inducing_locations: bool = True,
            jitter_val: Optional[float] = None,
    ):
        super().__init__(
            model, inducing_points_count, variational_distribution, learn_inducing_locations, jitter_val=jitter_val
        )
    ...
    def forward(self, x: Tensor, inducing_points_cont: Tensor, inducing_values_cont: Tensor,
                inducing_points_cls: Tensor, inducing_values_cls: Tensor,
                variational_inducing_covar: Optional[LinearOperator] = None,
                **kwargs) -> tuple[MultivariateNormal, MultivariateNormal]:
        # Compute full prior distribution
        full_inputs = torch.cat([inducing_points_cont, x], dim=-2)
        full_output_cont, full_output_cls = self.model.forward(full_inputs, **kwargs)

        full_covar_cont = full_output_cont.lazy_covariance_matrix
        full_covar_cls = full_output_cls.lazy_covariance_matrix

        predictive_mean_cont, predictive_covar_cont = self.get_predictive_dist(inducing_points_cont, inducing_values_cont,
                                                                               full_inputs,
                                                                               full_output_cont, full_covar_cont,
                                                                               variational_inducing_covar)
        predictive_mean_cls, predictive_covar_cls = self.get_predictive_dist(inducing_points_cls, inducing_values_cls,
                                                                             full_inputs,
                                                                             full_output_cls, full_covar_cls,
                                                                             variational_inducing_covar)

        return MultivariateNormal(predictive_mean_cont, predictive_covar_cont), MultivariateNormal(predictive_mean_cls,
                                                                                                   predictive_covar_cls)

    def get_predictive_dist(self, inducing_points, inducing_values, full_inputs, full_output, full_covar,
                            variational_inducing_covar):
        # Covariance terms
